...
import { products, getCategoryName, categorySlugToName, formatPrice } from "@/lib/products";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="font-heading text-3xl text-white mb-4">Category Not Found</h1>
            ...
            >
              <div className="relative overflow-hidden rounded-sm bg-secondary-bg border border-white/5 group-hover:border-luxury-gold/30 transition-all duration-500">
                ...
                <div className="absolute top-2 right-2">
                  <button
                    onClick={() => { toggleWishlist(product); showToast(isInWishlist(product.id) ? "Removed" : "Added to wishlist"); }}
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all ${
                      isInWishlist(product.id) ? "bg-luxury-gold text-primary-bg" : "glass text-light-gray hover:text-luxury-gold"
                    }`}
                  >
